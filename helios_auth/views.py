...
def after(request):
  ...
  #------------------------------------------------------------------------------------------------------------------    
  #------------------------------------------------------------------------------------------------------------------
  # log informações do usuário administrador após o login
  ...
  #obter data e hora do acesso
  data_hora=datetime.now().strftime('%d/%m/%Y %H:%M:%S')
  
  # verificar se user é um admin
  con = psycopg2.connect(host='localhost', database='helios',user='helios_log', password='#2023h')

  sql = 'select user_id from helios_auth_user where admin_p is true;'

  with con as conexao:
     sql_query = pd.read_sql_query(sql, con)
     df = pd.DataFrame(sql_query)

  if request.session['ldap_user']['username'] in df['user_id'].values:
    # montar linha do log
    linha=cast_ip+', '+data_hora+', '+request.session['ldap_user']['username']+', '+request.session['ldap_user']['name']+', admin\n'
  else:
    linha=cast_ip+', '+data_hora+', '+request.session['ldap_user']['username']+', '+request.session['ldap_user']['name']+'\n'
    
  #abrir e salvar linha no arquivo de log em disco
  data_dia=datetime.now().strftime('%d-%m-%Y')
  arquivo = open('/opt/log/log_acessos_helios_'+data_dia+'.txt','a')
  arquivo.write(unidecode(linha))
  #---------------------------------------------------------------------------------------------------------------------
  #---------------------------------------------------------------------------------------------------------------------
