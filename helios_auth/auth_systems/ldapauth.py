...
def save_admin_login(request):
    ...
    # montar linha do log
    linha=datetime.now()+', '+request.session['ldap_user']['username']+', '+request.session['ldap_user']['name']+','
    
    #salvar arquivo de log em disco
    arquivo = open('/opt/log/log_acesses_admin.txt','a')
    arquivo.write(linha)
