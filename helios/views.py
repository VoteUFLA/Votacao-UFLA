...
def elections_administered(request):
  if not can_create_election(request):
    return HttpResponseForbidden('somente um administrador tem eleições para administrar')
...
def election_new(request):
  if not can_create_election(request):
    return HttpResponseForbidden('somente um administrador pode criar uma eleição')
...
def trustee_login(request, election_short_name, trustee_email, trustee_secret):
  ...
  raise Http404("\c {} found.".format(election_short_name))
...
Você foi selecionado(a) como apurador(a) da %s.

A sua página de apurador pode ser acessada no endereço: %s
...
""" % (election.name, url)
  ...
  utils.send_email(settings.SERVER_EMAIL, ["%s <%s>" % (trustee.name, trustee.email)], 'ATENÇÃO: Você foi designado como apurador da eleição %s' % election.name, body)
...
def trustee_upload_pk(request, election, trustee):
  if request.method == "POST":
    ...
    try:
      election.admin.send_message("%s - upload da chave pública" % election.name, "O(A) Apurador(a) %s (%s) fez o upload da chave pública. " % (trustee.name, trustee.email))
    except:
...
def one_election_register(request, election):
  if not election.openreg:
    return HttpResponseForbidden('registro fechado para essa eleição')
    ...
    if bool(request.POST.get('confirm_p', 0)):
      ...
      if 'voters_file' in request.FILES:
        ...
        try:
          ...
          voters = []
          problems.append("o seu arquivo CSV não pôde ser processado. Por favor, verifique se é um arquivo CSV válido %s" % str(e))
