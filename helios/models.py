...
class Election(HeliosModel):
  ...
  def pretty_eligibility(self):
    if not self.eligibility:
      return "Qualquer eleitor que possa se conectar no sistema."
    else:
      return_val = "<ul>"
  ...
  def issues_before_freeze(self):
    ...
    if self.questions is None or len(self.questions) == 0:
      issues.append(
        {'type': 'questions',
         'action': "Adicionar questões à cédula"}
        )
    ...
    if len(trustees) == 0:
      issues.append({
          'type': 'trustees',
          'action': "Adicionar ao menos um apurador"
          })
    ...
    for t in trustees:
      if t.public_key is None:
        issues.append({
            ...
            'action': 'Apurador(a) %s gerar um par de chaves' % t.name
            })

    if self.voter_set.count() == 0 and not self.openreg:
      issues.append({
          "type" : "voters",
          "action" : 'informar lista de eleitores'
          })
  ...
  def freeze(self):
    ...
    if len(self.issues_before_freeze) > 0:
      raise Exception("não é possível congelar uma eleição com problemas")
  ...
  def process(self):
    ...
    for voter in voters:
      if voter['voter_type'] == 'password':
          ...
          user, _ = User.objects.get_or_create(user_type=voter['voter_type'], user_id=voter['voter_id'], defaults = {'name': voter['voter_id'], 'info':{'email':voter['email']}, 'token': None})
